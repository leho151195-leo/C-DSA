...
void donQueue(QUEUE& q) {
    int vitri = 0;
    for (int i=q.qFront; i<=q.qCapacity-1; i++) {
        q.Array[vitri++]=q.Array[i];
    }
    q.qRear=q.qRear-q.qFront;
    q.qFront=0;
    for (int i = q.qRear + 1; i < q.qCapacity; i++) {
        q.Array[i] = INT_MIN;
    }
}